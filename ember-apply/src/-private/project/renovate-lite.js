...
import { modify as modifyPackageJSON,read as readPackageJSON } from '../package-json/utils.js';
	...
	for (const workspace of workspaces) {
		...
		await inWorkspace(workspace, async () => {
			...
			for (const depType of DEP_TYPES) {
				Object.keys(manifest[depType] ?? {}).forEach((depName) => flat.add(depName));
			}

			const relevantDeps = Object.keys(resolved).filter((depName) => flat.has(depName));
