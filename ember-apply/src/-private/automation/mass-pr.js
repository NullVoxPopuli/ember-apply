...
export async function massPR(options) {
  ...
  assert(options.name, `options.name is required so that a PR title can be generated`);
  assert(options.description, `options.description is required so that a PR description can be generated`);
  assert(options.branch, `options.branch is required so that branches can be created`);
  assert(options.commit, `options.commit is required so that commits can have meaningful contributions to the git history`);
  assert(options.doWork, `options.doWork is required so that the actual work can be done`);
  assert(options.ensurePR, `options.ensurePR is required so that the PR can be created / updated`);
  ...
  for (let bucket of buckets) {
    ...
    await git.switchTo(branchName, { cwd, fromBranch: `${remote}/${baseBranch}` });
